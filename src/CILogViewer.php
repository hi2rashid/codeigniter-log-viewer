<?php
/**
 * Author: Abdul Rashid ( hi2rashid@gmail.com )
 * Date: 18-Jul-18
 * Time: 13:32 PM
 */
...
class CILogViewer {
    ...
    private $LOG_FOLDER_PREFIX = APPPATH . "/logs";
    private $FILE_PATH_PATTERN = APPPATH . "/logs/log-*.php";
    ...
    const LOG_VIEW_FILE_FOLDER = APPPATH . "views/cilogviewer"; //Changes done here
    ...
    public function __construct($path=NULL) {
        if($path){
            $this->LOG_FOLDER_PREFIX = $path;
            $this->FILE_PATH_PATTERN = $path. "/log-*.php";
        }
        $this->init();
    }
...
//               ]);
    ...
    private function getFiles($basename = true)
    {
        ...
        $files = glob($this->FILE_PATH_PATTERN);
    ...
    private function deleteFiles($fileName) {
        ...
        if($fileName == "all") {
            array_map("unlink", glob($this->FILE_PATH_PATTERN));
        }
        else {
